...
class KokoroTTSProvider:
    ...
    def __init__(
        ...
        self._model_handler = KokoroModelHandler(model_path=model_path, voices_dir=voices_dir, use_gpu=use_gpu)

        self._voice_manager = KokoroVoiceManager(provider_id=self.provider_id, default_language=default_language)
    ...
    async def generate_speech(self, text: str, voice_id: str, options: Optional[Dict[str, Any]] = None) -> AudioResult:
        ...
        try:
            ...
            return await self._audio_generator.generate_speech(text=text, voice_id=voice_id, options=options)
