...
from ..exceptions import (
    CacheIOError, CachePermissionError, CacheDiskFullError, CacheError
)
...
class FileStorage:
    ...
    async def read(self, key: str) -> Optional[bytes]:
        ...
        try:
            ...
            raise CachePermissionError(f"Permission denied when reading from cache: {str(e)}", 
                                       {"key": key, "file_path": str(file_path)}) from e
        except OSError as e:
            logger.error(f"Cache read I/O error: {str(e)}")
            raise CacheIOError(f"I/O error during cache read: {str(e)}",
                              {"key": key, "file_path": str(file_path)}) from e
        except Exception as e:
            logger.exception(f"Unexpected cache read error: {str(e)}")
            raise CacheError(f"Unexpected error during cache read: {str(e)}",
                            {"key": key}) from e
    ...
    async def write(self, key: str, data: bytes) -> bool:
        ...
        try:
            ...
            raise CachePermissionError(f"Permission denied when writing to cache: {str(e)}", 
                                      {"key": key, "file_path": str(file_path) if file_path else "unknown"}) from e
        except OSError as e:
            ...
            if hasattr(e, 'errno') and e.errno == 28:
                ...
                raise CacheDiskFullError(f"Insufficient disk space for cache write: {str(e)}",
                                        {"key": key, "file_path": str(file_path) if file_path else "unknown"}) from e
            else:
                logger.error(f"Cache write I/O error: {str(e)}")
                raise CacheIOError(f"I/O error during cache write: {str(e)}",
                                 {"key": key, "file_path": str(file_path) if file_path else "unknown"}) from e
        except Exception as e:
            logger.exception(f"Unexpected cache write error: {str(e)}")
            raise CacheError(f"Unexpected error during cache write: {str(e)}",
                           {"key": key}) from e
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            ...
            raise CachePermissionError(f"Permission denied when deleting from cache: {str(e)}", 
                                      {"key": key, "file_path": str(file_path)}) from e
        except OSError as e:
            logger.error(f"Cache delete I/O error: {str(e)}")
            raise CacheIOError(f"I/O error during cache delete: {str(e)}",
                             {"key": key, "file_path": str(file_path)}) from e
        except Exception as e:
            logger.exception(f"Unexpected cache delete error: {str(e)}")
            raise CacheError(f"Unexpected error during cache delete: {str(e)}",
                           {"key": key}) from e
    ...
    async def clear(self) -> bool:
        ...
        try:
            ...
            raise CachePermissionError(f"Permission denied when clearing cache: {str(e)}", 
                                      {"cache_dir": str(self.cache_dir)}) from e
        except OSError as e:
            logger.error(f"Cache clear I/O error: {str(e)}")
            raise CacheIOError(f"I/O error during cache clear: {str(e)}",
                             {"cache_dir": str(self.cache_dir)}) from e
        except Exception as e:
            logger.exception(f"Unexpected cache clear error: {str(e)}")
            raise CacheError(f"Unexpected error during cache clear: {str(e)}",
                           {"cache_dir": str(self.cache_dir)}) from e
