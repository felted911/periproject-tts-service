...
class MetadataManager:
            ...
            try:
                ...
                try:
                    ...
                    raise CachePermissionError(
                        f"Permission denied when loading metadata: {str(e)}", {"file_path": str(self.metadata_file)}
                    ) from e
                except OSError as e:
                    logger.error(f"Metadata load I/O error: {str(e)}")
                    raise CacheIOError(
                        f"I/O error during metadata load: {str(e)}", {"file_path": str(self.metadata_file)}
                    ) from e
                ...
                try:
                    ...
                    raise CacheSerializationError(
                        f"Failed to parse metadata JSON: {str(e)}", {"file_path": str(self.metadata_file)}
                    ) from e
        ...
        async with self._lock:
            try:
                ...
                try:
                    ...
                    raise CacheSerializationError(
                        f"Failed to serialize cache metadata: {str(e)}", {"metadata_size": len(self.metadata)}
                    ) from e
                ...
                try:
                    ...
                    raise CachePermissionError(
                        f"Permission denied when saving metadata: {str(e)}", {"file_path": str(temp_file)}
                    ) from e
                except OSError as e:
                    ...
                    if hasattr(e, "errno") and e.errno == 28:
                        ...
                        raise CacheDiskFullError(
                            f"Insufficient disk space for metadata: {str(e)}", {"file_path": str(temp_file)}
                        ) from e
                    else:
                        logger.error(f"Cache metadata I/O error: {str(e)}")
                        raise CacheIOError(
                            f"I/O error during metadata save: {str(e)}", {"file_path": str(temp_file)}
                        ) from e
                ...
                try:
                    ...
                    raise CacheIOError(
                        f"Failed to rename metadata file: {str(e)}",
                        {"temp_file": str(temp_file), "metadata_file": str(self.metadata_file)},
                    ) from e
    ...
    def _clean_expired_items(self) -> List[str]:
        ...
        """
        now = time.time()
        expired_keys = [key for key, item in self.metadata.items() if now > item.get("expires_at", 0)]
