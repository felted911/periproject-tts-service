...
class CacheManager(CacheManagerProtocol):
    ...
    async def _force_eviction(self, required_space: int) -> None:
        ...
        try:
            ...
            items = sorted(self._metadata.metadata.items(), key=lambda item: item[1]["last_accessed"])
    ...
    async def _ensure_cache_size(self, new_data_size: int) -> None:
        ...
        try:
            ...
            keys_to_evict = self._eviction_policy.get_items_to_evict(
                self._metadata.metadata, new_data_size, self.max_size  # Pass full metadata dictionary
            )
