...
def test_validate_valid_request():
    ...
    request = TTSRequest(text="This is a test", voice="test_voice", options={}, format=AudioFormat.WAV)
...
def test_validate_text_too_long():
    ...
    request = TTSRequest(
        text="This text is too long for validation", voice="test_voice", options={}, format=AudioFormat.WAV
    )
...
def test_validate_uses_settings_value():
    ...
    with patch("src.tts_service.api.validators.tts_validator.settings") as mock_settings:
        ...
        # Valid request
        valid_request = TTSRequest(text="Short text", voice="test_voice", options={}, format=AudioFormat.WAV)
