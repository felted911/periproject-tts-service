...
@pytest.mark.parametrize(
    "endpoint_url",
    [f"{settings.API_PREFIX}/tts/", f"{settings.API_PREFIX}/tts"],  # With trailing slash  # Without trailing slash
)
def test_text_length_validation(client, test_tts_validator, endpoint_url):
    ...
    try:
        ...
        request_data = {"text": long_text, "voice": "test_voice", "format": "wav"}
        ...
        assert any(
            "text length exceeds maximum" in error.get("msg", "").lower() for error in response_json.get("detail", [])
        )
